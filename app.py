...
def update_graph(n):
    ...
    if df is None or df.empty:
        ...
        return fig, "Carregando...", {'color': '#888', 'textAlign': 'center'}, f"Iniciado: {hora_str}"
    
    score = df['RASTRO_AZUL'].iloc[-1]
    cor_score = "#00ff00" if score > 0 else "#ff0000"
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df['CINZA_NEUTRA'], line=dict(color='gray', width=2, dash='dash')))
    fig.add_trace(go.Scatter(x=df.index, y=df['VERMELHA'], line=dict(color='#ff0000', width=3)))
    fig.add_trace(go.Scatter(x=df.index, y=df['VERDE'], line=dict(color='#00ff00', width=3)))
    fig.add_trace(go.Scatter(x=df.index, y=df['RASTRO_AZUL'], line=dict(color='#00bcff', width=6, dash='dot')))
    ...
    fig.update_layout(
        ...
        xaxis=dict(gridcolor='#222', tickformat='%H:%M', title='Horário (Brasília UTC-3)'),
        showlegend=False
    )
...
# ✅ CORRIGIDO: app.run ao invés de app.run_server
